...
type BusStatus = "In Transit" | "Stopped" | "Delayed" | string;
...
export default function BusStatusList({ buses, loading }: BusStatusListProps) {
  if (loading) {
  ...
  if (!buses.length) {
    return (
      <div className="empty-state">
        No buses reporting a location right now.
      </div>
    );
  }
        ...
        <div className="bus-item" key={`${bus.region}-${bus.id}`}>
          ...
          >
            <div className="bus-coords">
              {bus.latitude?.toFixed(5) ?? "0.00000"},{" "}
              {bus.longitude?.toFixed(5) ?? "0.00000"}
            </div>
